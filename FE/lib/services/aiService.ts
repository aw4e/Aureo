...
import { getGoldPrice } from './contractService';
...
export async function analyzeGoldMarket(
  ...
  try {
    // Get current gold price from smart contract (Pyth Oracle)
    let currentPrice: number;
    try {
      currentPrice = await getGoldPrice();
    } catch {
    ...
    const prompt = `You are AUREO AI, an expert gold trading analyst for tokenized gold (mGold).
...
export async function chatWithAI(
  ...
  try {
    const prompt = `You are AUREO AI, a friendly and knowledgeable gold investment assistant.

USER PORTFOLIO:
- Gold Balance: ${goldBalance.toFixed(6)} mGold
- USDC Balance: $${usdcBalance.toFixed(2)}
- Current Gold Price: $${goldPrice.toFixed(2)}
- Portfolio Value: $${(goldBalance * goldPrice + usdcBalance).toFixed(2)}
...
export async function getMarketInsight(goldPrice: number): Promise<string> {
  try {
    const prompt = `As AUREO AI, provide a brief 1-2 sentence market insight for gold at $${goldPrice.toFixed(2)}.
