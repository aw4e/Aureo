...
# Alternative verification using flattened contracts
# This avoids the need to rebuild and uses pre-flattened files
...
# Verify MockUSDC using flattened file
echo "📝 [1/3] Verifying MockUSDC at $USDC_ADDR..."
$FORGE verify-contract $USDC_ADDR \
    MockTokens.flattened.sol:MockUSDC \
    --chain-id 5003 \
    --verifier blockscout \
    --verifier-url https://explorer.sepolia.mantle.xyz/api || echo "⚠️  MockUSDC verification failed or already verified"

echo ""

# Verify MockGold using flattened file  
echo "📝 [2/3] Verifying MockGold at $GOLD_ADDR..."
$FORGE verify-contract $GOLD_ADDR \
    MockTokens.flattened.sol:MockGold \
    --chain-id 5003 \
    --verifier blockscout \
    --verifier-url https://explorer.sepolia.mantle.xyz/api || echo "⚠️  MockGold verification failed or already verified"

echo ""

# Verify AureoRWAPool using flattened file with constructor args
...
$FORGE verify-contract $POOL_ADDR \
    AureoRWAPool.flattened.sol:AureoRWAPool \
    --constructor-args $CONSTRUCTOR_ARGS \
    --chain-id 5003 \
    --verifier blockscout \
    --verifier-url https://explorer.sepolia.mantle.xyz/api || echo "⚠️  AureoRWAPool verification failed or already verified"
